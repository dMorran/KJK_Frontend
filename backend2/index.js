...
app.post("/upload", upload.single("file"), (req, res) => {
  ...
  if (
    ...
    try {
      if (exifData.GPS) {
        exifData.GPS["2"][0][0] = addFromLength(
          exifData.GPS["2"][0][0],
          parseInt(coordAdd)
        );
        exifData.GPS["4"][0][0] = addFromLength(
          exifData.GPS["4"][0][0],
          parseInt(coordAdd)
        );
      }
    ...
    const editedImageData = px.insert(
      px.dump(exifData),
  ...
  const fileLink =
    "http://localhost:3000/download/" + newFileName + fileExtension;
...
app.post("/original/:filename", upload.single("file"), (req, res) => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    if (
      ...
      try {
        if (exifData.GPS) {
          exifData.GPS["2"][0][0] = subtractFromLength(
            exifData.GPS["2"][0][0],
            parseInt(coordAdd)
          );
          exifData.GPS["4"][0][0] = subtractFromLength(
            exifData.GPS["4"][0][0],
            parseInt(coordAdd)
          );
        }

        exifData["0th"]["306"] = decrypt(
          exifData["0th"]["306"],
          keyBuffer,
          ivBuffer
        );
        exifData.Exif["36867"] = decrypt(
          exifData.Exif["36867"],
          keyBuffer,
          ivBuffer
        );
        exifData.Exif["36868"] = decrypt(
          exifData.Exif["36868"],
          keyBuffer,
          ivBuffer
        );
        exifData["0th"]["271"] = decrypt(
          exifData["0th"]["271"],
          keyBuffer,
          ivBuffer
        );
        exifData["0th"]["272"] = decrypt(
          exifData["0th"]["272"],
          keyBuffer,
          ivBuffer
        );
